...
// based on https://github.com/lukechampine/uint128

// ZeroUint128 is a zero-valued uint128.
...
// Add returns u+v. No error, if the result overflows
...
// Add64 returns u+v. No error, if the result overflows
...
// Sub returns u-v. No error, if the result overflows
...
// Sub64 returns u-v. No error, if the result overflows
...
// Mul returns u*v. No error, if the result overflows
...
// Mul64 returns u*v. No error, if the result overflows
...
// Div64 returns u/v. No error, if the result overflows
...
func (u Uint128) Format(f fmt.State, verb rune) {
	...
	if string(verb) == "d" {
		...
		fr := fmt.Sprintf("%s%s%s%s", "%", string(verb), "%", string(verb))
		s = fmt.Sprintf(fr, u.Hi, u.Lo)
	}
